...
df=pd.read_excel('output_file2.xlsx', sheet_name='Sheet1')
...
df = pd.DataFrame({
    "date_column": pd.date_range(start="2024-01-01", periods=100, freq="D"),
    "value": [x**0.5 for x in range(100)]  # Sample numerical data
})
...
# Sidebar Filters
year_selection = st.sidebar.multiselect("Select Year", options=df["Year"].unique(), default=df["Year"].unique())
month_selection = st.sidebar.multiselect("Select Month", options=df["Month"].unique(), default=df["Month"].unique())

# Apply Filters
df_filtered = df.query("Year == @year_selection & Month == @month_selection")

# Create Histogram
fig = px.histogram(df_filtered, x="value", nbins=12, title="Histogram Chart")
...
# Sidebar filters
company_or = st.sidebar.multiselect(
    "Хувь хүн эсэх",
    options=df["Хувь/Байгууллага"].unique(),
    default=df["Хувь/Байгууллага"].unique(),
)

car_or = st.sidebar.multiselect(
    "Автомашин эсэх",
    options=df["Автомашин эсэх"].unique(),
    default=df["Автомашин эсэх"].unique(),
)
...
# Rename column
df.rename(columns={'Хувь/Байгууллага': 'Хувь_Байгууллага'}, inplace=True)

# Apply filtering
df_selection = df.query(
    "`Хувь_Байгууллага` == @company_or & `Автомашин эсэх` == @car_or & `Year` == @year_selection & `Month` == @month_selection"
)
...
def Home():
    with st.expander("VIEW EXCEL DATASET"):
        showData=st.multiselect('Filter: ',df_selection.columns,default=['date_column', 'Регистр', 'Нэр', 'Хувь_Байгууллага', 'Тээврийн төрөл_2',
       'Мэдүүлэгч', 'Илгээгч', 'Автомашин эсэх', 'гадаад нэр', 'Нэгж', 'Тоо',
       'ГХН', 'Year', 'Month'])
